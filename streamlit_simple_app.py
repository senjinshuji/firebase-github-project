...
with st.sidebar:
    ...
    with st.expander("🔧 管理者機能"):
        ...
        # GitHubに保存
        st.divider()
        if st.button("☁️ GitHubに保存", help="data.jsonをGitHubにアップロード"):
            import subprocess
            try:
                # Git add
                ...
                subprocess.run(["git", "commit", "-m", f"Update data.json from Streamlit - {timestamp}"], check=True)
                # Git push
                subprocess.run(["git", "push", "origin", "main"], check=True)
                st.success("✅ GitHubに保存しました！")
            except subprocess.CalledProcessError as e:
                st.error(f"❌ エラー: {str(e)}")
                st.info("ローカル環境でのみ動作します")